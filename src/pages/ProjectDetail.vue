...
<template>
  <div class="dark bg-gray-900 min-h-screen relative overflow-hidden">
    ...
    <section v-if="project" class="relative z-10 container mx-auto py-32 px-6 text-white">
      ...
      <!-- Sections dynamiques -->
      <div v-if="project.sections && project.sections.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 text-left text-gray-200 mb-16">
        <div v-for="(section, si) in project.sections" :key="si">
          <h3 class="text-xl font-bold text-purple-400 mb-2">{{ section.title }}</h3>
      ...
      <!-- Images multiples -->
      <div v-if="project.images && project.images.length > 0" class="flex flex-wrap gap-6 justify-center mb-12">
        <img v-for="(img, i) in project.images" :key="i" :src="img" :alt="project.title" class="rounded-xl shadow-lg max-w-sm" />
      </div>
